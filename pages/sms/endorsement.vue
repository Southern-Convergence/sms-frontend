<template>
  <v-sheet>
    ...
    <v-dialog v-model="endorsement_log_dialog" max-width="600px">
      <v-card>
        <v-toolbar color="primary" border>
        ...
        <v-card-text>
          <v-alert color="primary" variant="outlined" v-for="log in logs_data.selectable.endorsement_log" :key="log"
            ...
            <v-card>
              ...
              <v-card-subtitle class="d-flex align-center mb-1">
                <v-icon color="green darken-2" class="mr-2">mdi-account</v-icon>
                {{ log.status === 'Verified' ? 'Verified by' : 'Generated by' }}: <b class="ml-1"> {{ log.signatory
                  }}</b>
              </v-card-subtitle>
              ...
              <!-- Optional: Reason/Remarks Section with Icon -->
              <v-card-subtitle v-if="log.remarks" class="d-flex align-center">
                <v-icon color="red darken-2" class="mr-2">mdi-comment-alert</v-icon>
                Remarks:
                <v-chip color="red lighten-2" class="ml-2">{{ log.remarks }}</v-chip>
              </v-card-subtitle>
            </v-card>
          </v-alert>
        </v-card-text>
