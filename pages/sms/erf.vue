  ...
  <div>
    ...
    <v-card-text>
      ...
      <v-row no-gutters> <v-col cols="5" class="text-subtitle-2 font-weight-bold"> DIVISION : {{
        ...
        <v-spacer />
        <v-col cols="auto" v-if="is_endorsement_letter"> <v-btn @click="applicant_endorsement(applicant_details._id)"
            color="amber" block density="compact"> View
      ...
      <v-row no-gutters>
        <!-- <v-col cols="12" class="mt-4">
          <v-alert border="top" variant="outlined" type="error" v-if="applicant_details?.request_log?.length">
            <template v-slot:title> ACTION TAKEN </template>
<v-list lines="one" class="pl-6" flat v-for="(attachment, index) in applicant_details?.request_log" :key="index">
  <v-list-title class="pl-4" v-if="attachment?.remarks"> Evaluated by : <u>{{
      attachment.signatory
      }}</u></v-list-title>
  <v-list-item density="compact" v-for="(att, index) in attachment.remarks" :key="index">
    <template v-if="att?.remarks">
                  <v-list-title> ATTACHMENT : <b>{{ att.description }}</b></v-list-title>
                  <v-list-item-subtitle>
                    <span class="text-red">REASON</span> : {{ att.remarks }}
                  </v-list-item-subtitle>
                </template>
  </v-list-item>
</v-list>
<v-divider />
          ...
          <v-card class="mx-auto mt-4" rounded="lg">
            <v-card-title class="d-flex  font-weight-bold">
              {{ applicant_details.is_with_erf === true ? '1. EQUIVALENT RECORD FORM ' : 'Applicant Details'
                  ...
                  HISTORY</v-btn></span>
              <span> <v-btn v-if="applicant_details.is_with_erf" @click="applicant_erf(applicant_details._id)"
        ...
        <v-col cols="12" class="my-5" v-if="applicant_details.status === 'For DBM'">
    ...
    <v-card-actions>
      <v-row justify="center">
        ...
        <v-col cols="6" v-else-if="user.role === ROLES.ADMIN5 && applicant_details?.status === 'Completed'">
          <v-btn @click="ro_evaluator_dialog = true" block variant="tonal" color="indigo"> Assign to Evaluator</v-btn>
        </v-col>
        <v-col cols="6"
        ...
        <v-col cols="6" v-else-if="applicant_details?.status === 'For DBM'"><v-btn color="primary" block
            @click="complete_reclass">Complete Reclassification</v-btn></v-col>
        <v-col cols="6" v-else>
...
const is_endorsement_letter = computed(() => {
  const x = ["Verifier", "Evaluator", "Administrative Officer V"].includes(user.role);
  const y = user.side === 'RO';
  return x && y;
});
...
const display_sdo_attachment_conditon = computed(() => {
  ...
  const evaluator = user.role === 'Evaluator' && user.side === 'SDO'
  const principal = user.role === 'Principal'
  return admin4 || principal || evaluator
})

const sdo_attachment_evaluator_condition = computed(() => {
  const admin5 = (user.role === 'Administrative Officer V' || user.role === 'Administrative Officer IV') && applicant_details.value.status === 'Pending'
  const ro_verifier = user.role === 'Verifier' && user.side === 'RO'
  return admin5 || ro_verifier
})
