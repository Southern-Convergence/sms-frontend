  ...
  <v-layout>
    <v-bottom-sheet temporary inset>
      <template v-slot:activator="{ props }">
        <v-row no-gutters>
          <v-btn prepend-icon="mdi-upload" color="grey-darken-2" v-bind="props" text="Upload E-signature"></v-btn>
        </v-row>
      </template>
      <v-card>
        <v-toolbar color="primary">
          <v-row>
            <v-col cols="3" class="text-uppercase text-white text-h6 pl-5"> <v-icon class="pr-2"
                color="#E3F2FD">mdi-draw-pen</v-icon>Signatures</v-col>
            <v-col cols="9" class="text-white text-h6"> E-Signatures</v-col>
          </v-row>
        </v-toolbar>
        <v-sheet>
          <v-row no-gutters>
            <v-col cols="3">
              <v-sheet>
                <v-card-text class="text-center" style="height: 350px; overflow-y: scroll">
                  <div v-for="sig, sig_index in get_signatures" :key="sig_index">
                    <img width="100" class="preview cursor" :src="sig.link" @click="select_esig(sig)" />
                  </div>
                </v-card-text>
              </v-sheet>
            </v-col>
              ...
              <v-window v-model="tab">
                <v-window-item :value="1">
                  <v-row no-gutters justify="center">
                    <v-col cols="10">
                      <v-card class="w-100 ma-2 d-flex align-center justify-center" variant="tonal" height="290">
                  ...
                  <v-divider />
                  <v-card-actions>
                    <v-row dense v-if="imageData === ''"> <v-btn color="primary" variant="tonal"
                        prepend-icon="mdi-upload" class="custom-file-upload"
                        onclick="document.getElementById('getFile').click()">Select
                        File</v-btn>
                ...
                <v-window-item :value="2">
                  <v-row no-gutters justify="center">
                    <v-col cols="10">
                      <v-card class="ma-2" variant="tonal" height="290">
                  ...
                  <v-card-actions>
                    <v-row justify="center">
                      <v-col cols="2"> <v-btn color="red" variant="tonal" density="compact" block
                          @click="clear">Clear</v-btn></v-col>
                      <v-col cols="2"> <v-btn class="custom-file-upload" block density="compact" color="primary"
